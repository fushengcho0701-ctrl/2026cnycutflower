...
function renderProducts() {
  ...
  allProducts.forEach(p => {
    ...
    const isPhal = (p.series || "").trim() === "Phalaenopsis 蝴蝶蘭切花";
    const subText = isPhal ? "(12支/箱)" : "(最少起訂 4 把)";

    card.innerHTML = `
      <div class="product-img-wrap" data-fullsrc="${p.imageUrl}"><img src="${p.imageUrl}" loading="lazy" /></div>
      <div class="product-name">${p.name} <br><small style="color:#777; font-weight:normal;">${subText}</small></div>
      <div class="product-price">HKD$${p.price}</div>
      <div class="product-qty"><input type="number" min="0" value="0" data-name="${p.name}" data-price="${p.price}" data-series="${p.series || ''}" /></div>`;
...
async function handleSubmit() {
  ...
  const items = [];
  let total = 0, belowMin = [];

  document.querySelectorAll(".product-qty input").forEach(input => {
    const qty = parseInt(input.value || "0", 10);
    const series = (input.dataset.series || "").trim();
    if (qty > 0) {
      if (series !== "Phalaenopsis 蝴蝶蘭切花" && qty < 4) belowMin.push(input.dataset.name);
